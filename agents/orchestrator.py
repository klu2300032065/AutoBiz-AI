...
class Orchestrator:
    ...
    def determine_next_action(self):
        ...
        pending_apps = self.approval_manager.check_pending_approvals()

        if pending_apps:
            return "WAIT FOR APPROVAL"

        # Check existing pending tasks
        next_task = self.task_manager.get_next_runnable_task()
        if next_task:
            return next_task.type

        if state.product is None and state.research_results is None:
            # Need to research
            self.task_manager.create_task("RESEARCH_PRODUCT", "Research a new product opportunity")
            return "RESEARCH_PRODUCT"
            
        if state.research_results is not None and state.product is None and state.stage == "RESEARCH_COMPLETED":
            # Research done, wait for approval
            self.approval_manager.request_approval(
                "APPROVE_PRODUCT",
                "Approve the researched product idea for building.",
                risk_level="MEDIUM",
                estimated_cost=0.0
            )
        ...
        if state.stage == "PRODUCT_APPROVED" and state.build_status is None:
            # Build product
            self.task_manager.create_task("BUILD_PRODUCT", f"Build the product: {state.product}")
            return "BUILD_PRODUCT"

        if state.build_status == "COMPLETED" and state.qa is None:
            # Run QA
            self.task_manager.create_task("RUN_QA", f"Run QA on {state.product}")
            return "RUN_QA"

        if state.qa == "FAIL":
            # Need to rebuild/fix
            self.task_manager.create_task("BUILD_PRODUCT", f"Fix QA issues for {state.product}")
            return "BUILD_PRODUCT"

        if state.stage == "QA_COMPLETED" and state.deployment is None:
            # Prepare deployment
            self.task_manager.create_task("PREPARE_DEPLOYMENT", f"Prepare deployment for {state.product}")
        ...
        if state.stage == "DEPLOYMENT_APPROVED" and state.marketing is None:
            # Run Marketing
            self.task_manager.create_task("CREATE_MARKETING", f"Create marketing for {state.product}")
